<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	{		
		$CI =& get_instance();
                ...
                if($CI->session->userdata('hr_id')!='')
                {
                    //check user active
                    $a_data = get_one_admin($CI->session->userdata('hr_id'));
	...
	{
		$CI =& get_instance();
		$query = $CI->db->get_where('tblhr',array('hr_id'=>$id));
		return $query->row();
	}	
	// --------------------------------------------------------------------
	...
	{		
		$CI =& get_instance();
		return $CI->session->userdata('hr_id');
	}
	...
	function email_send($email_address_from,$email_address_reply,$email_to,$email_subject,$str)
	{
	  ...
	  echo $email_subject;	
	   echo $str;
		...
		$email_set=$query->row();
		//echo "<pre>";print_r($email_set);die; 			
		...
		$CI->email->from($email_address_from,"Payroll system");
