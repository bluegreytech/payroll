        ...
        <div class="row">
            <div class="col-md-12 card-box mb-0">
                <div class="table-responsive m-t-15">
                            <form method="POST" action="<?php echo base_url();?>adminmaster/assign_admin_rights/<?php echo $admin_id ?>">
                            <table class="table">
                                            <thead>
                                                <tr>
                                           ...
                                           //  echo "<pre>";print_r($rid->rights_add);
                                                ...
                                                foreach($all_rights as $row){
                                                 // print_r($row);
                                                  $rid=  $row->admin_rights_id;
                                                 $admin_id=$this->uri->segment(3);
                                                 $rrr=getadminrights($rid,$admin_id);
                                                    if(!empty($rrr)){
                                                       foreach ($rrr as $val) {
                                                           # code...
                                                     // echo "<pre>njhg"; print_r($val);
                                                ?>
                                                <input type="hidden" name="rightid[]" value="<?php echo $row->admin_rights_id; ?>">
                                                 <input type="hidden" name="assignrightid[]" value="<?php  if(!empty($rid[$row->admin_rights_id])) { echo $rid[$row->admin_rights_id]->rights_id; } ?>">
                                                
                                                    <td><?php echo ucfirst($row->rights_name);?></td>
                                                     <td class="text-center">
                                                      <input type="checkbox" name="addrightcheck[<?php echo $row->admin_rights_id;?>]" value="1" <?php if($val->rights_add=='1'){ echo "checked";} ?>>
                                                    </td>
                                                     <td class="text-center">
                                                      <input  type="checkbox" name='updaterightcheck[<?php echo $row->admin_rights_id;?>]' value="1" <?php if($val->rights_update=='1'){ echo "checked";}?>>
                                                    </td>
                                                    <td class="text-center">
                                                      <input  type="checkbox"   name="deleterightcheck[<?php echo $row->admin_rights_id;?>]" value="1" <?php if($val->rights_delete=='1'){ echo "checked";} ?>>
                                                    </td>
                                                     <td class="text-center">
                                                      <input  type="checkbox"   name="viewrightcheck[<?php echo $row->admin_rights_id;?>]" value="1" <?php if($val->rights_view=='1'){ echo "checked";} ?>>
                                                    </td>
                                                </tr>
                                               <?php  } }else{ ?>
                                                 <input type="hidden" name="rightid[]" value="<?php echo $row->admin_rights_id; ?>">
                                                 ...
                                                 <input type="hidden" name="assignrightid[]" value="<?php if(!empty($rid[$row->admin_rights_id])) { echo $rid[$row->admin_rights_id]->rights_id; } ?>">
                                                    ...
                                                    <td><?php echo ucfirst($row->rights_name);?></td>
                                                     <td class="text-center">
                                                      <input  type="checkbox" name="addrightcheck[<?php echo $row->admin_rights_id;?>]" value="1" >
                                                    </td>
                                                     <td class="text-center">
                                                      <input  type="checkbox" name='updaterightcheck[<?php echo $row->admin_rights_id;?>]' value="1" >
                                                    </td>
                                                    <td class="text-center">
                                                      <input  type="checkbox" name="deleterightcheck[<?php echo $row->admin_rights_id;?>]" value="1" >
                                                    </td>
                                                     <td class="text-center">
                                                      <input  type="checkbox" name="viewrightcheck[<?php echo $row->admin_rights_id;?>]" value="1" >
                                                    </td>
                                                </tr>
                            ...
                            <div class="submit-section" style="margin-top: 0px;">
                                    ...
                                    <button type="button" name="cancel" class="btn  btn-rights" onclick="location.href='<?php echo base_url()?>adminmaster/adminlist'"> Cancel </button>
                            </div>  
