...

{
	...
	{	
		...
		$this->db->where_in('t1.Companynotificationid',$insert_id);
		// $smtp2 = $this->db->get();
		// $res = $smtp2->result();
		// return $res;
		...
		foreach($smtp2->result() as $rows) {
			...
			foreach($userid as $usid)
			{
				...
				$smtp3 = $this->db->get();
				foreach($smtp3->result() as $rowss) {
				echo	$companyid= $rowss->companyid;
				echo	$companyname= $rowss->companyname;
				echo	$comemailaddress= $rowss->comemailaddress;
				}
			}
		}
		die;
		...
		// $selectId=mysqli_query($con,"SELECT * from tblnotification
		// 	where NotificationId IN (".$NotificationId.")");
		// 	$datauser=mysqli_fetch_array($selectId);
		// 	//$videos['UserId'];
		// 	$userid=explode(",",$datauser['UserId']);
		// 	foreach($userid as $usid)
